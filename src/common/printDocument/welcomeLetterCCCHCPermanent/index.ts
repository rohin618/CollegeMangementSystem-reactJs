...
import { roomData } from '../../data/rooms/room.data';
import { getActiveFundDetailsByJointFund } from '../../../helpers/resident';

export const welcomeLetterCCCHCPermanent = (
    residentData: any,
    comapanyDetails: any,
    ...
    const laMonthlyPrice = evaluateMonthlyPrice(
        ...
        isClientContribution ? activeFund?.clientContribution : activeFund?.familyTopupPrice
    )
    const laDescription = isClientContribution ?
        billingFormulas.find((item: any) => item.id === comapanyDetails?.ccBillingPattern).description :
        billingFormulas.find((item: any) => item.id === comapanyDetails?.familyTopupPattern).description;
...
">
        ...
        <p>We understand <b>${getLabelByValue(SALUTATION_LIST, residentData?.personal?.salutation)}. ${residentData?.personal?.name} </b>will be in receipt of funding from <b>${fundingName}</b> (${isClientContribution ? priceFormat(roomPrice - activeFund?.clientContribution || 0) : priceFormat(roomPrice - activeFund?.familyTopupPrice || 0)} per week). The weekly ${isClientContribution ? 'Client Contribution' : 'Family Topup'} is <strong>${isClientContribution ? priceFormat(activeFund?.clientContribution || 0) : priceFormat(activeFund?.familyTopupPrice || 0)}</strong> excluding FNC. The Monthly invoice will be <strong>${priceFormat(laMonthlyPrice)}</strong> for <b>${getLabelByValue(SALUTATION_LIST, residentData?.personal?.salutation)}. ${residentData?.personal?.name}</b> (${isClientContribution ? priceFormat(activeFund?.clientContribution || 0) : priceFormat(activeFund?.familyTopupPrice || 0)} ${laDescription}).</p>
...
"
class="welcomeLetter-footer permanentCC-welcomeLetter"
>
