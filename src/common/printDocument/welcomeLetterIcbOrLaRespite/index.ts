...
export const welcomeLetterIcbOrLaRespite = (residentData: any, comapanyDetails: any, roomInfo: any, localAuthorityList: any, localICBList: any, theme: any, headOfficeAddress: any, logoBase64: any) => {
...
${todayFormatted}</div>
        ...
        <p>We understand <b>${getLabelByValue(SALUTATION_LIST, residentData?.personal?.salutation)}. ${residentData?.personal?.name}</b> will be in receipt of funding from <b>${fundingName}</b>. The weekly nursing fee is ${priceFormat(roomPrice)} excluding FNC. The Respite fee will be <strong>${priceFormat(totalAmount)}</strong> for <b>${getLabelByValue(SALUTATION_LIST, residentData?.personal?.salutation)}. ${residentData?.personal?.name}</b> (${priceFormat(roomPrice)} per week divided by 7 days, multiplied by <strong>${totalDays} days</strong>). Please note all payments will be fully funded by the <b>${fundingName}</b>.</p>

        <p>Please find attached the contract for <b>${getLabelByValue(SALUTATION_LIST, residentData?.personal?.salutation)}. ${residentData?.personal?.name}</b>. Please sign and return one copy of the contract within 5 working days by email or post it to ${comapanyDetails?.tradeName} ${comapanyDetails.buildingNumber} ${comapanyDetails?.area} ${comapanyDetails?.address} ${comapanyDetails?.postCode}. If we do not receive the signed contract within 5 working days, we will deem you are in agreement with our contract.</p>
...
"
class="welcomeLetter-footer mt-4 RespiteLetter-footer"
>
