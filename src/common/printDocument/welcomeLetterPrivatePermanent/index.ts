...
import { evaluateMonthlyPrice, getActiveFundDetails, getActiveRespiteDetails, getGender, getLabelByValue, priceFormat } from "../../../helpers/helpers";
import { SALUTATION_LIST } from "../../data/option";
import { LPA_TYPE, NOK_INVOICE_REQUIRED } from "../../constant";
import useDarkMode from "../../../hooks/useDarkMode";


export const welcomeLetterPrivatePermanent = (residentData: any, comapanyDetails: any, roomInfo: any, primaryBankDetails: any,theme:any,billingFormulas:any,headOfficeAddress:any,logoBase64:any) => {
    ...
    const formattedMonthDate = displayDate.isValid()
	? displayDate.format('MMMM - YYYY')
	: '';


    const privateMonthlyPrice = evaluateMonthlyPrice(
        billingFormulas.find((item:any) => item.id === comapanyDetails?.privateBillingPattern).formula,
        roomInfo?.perWeek,
    );

    const formulaDescription = billingFormulas.find((item:any) => item.id === comapanyDetails?.privateBillingPattern)?.description || '';
...
${todayFormatted}</div>
        ...
        <p>Please find enclosed the invoice for the month of <strong>${formattedMonthDate}
</strong> and an invoice for a one-month deposit (please refer to section 4 (4.1) in the contract). The weekly nursing fee is <strong>${priceFormat(roomInfo?.perWeek)}</strong> excluding FNC. The monthly invoice will be <strong>
        ${priceFormat(privateMonthlyPrice)}</strong> for <b>${getLabelByValue(SALUTATION_LIST, residentData?.personal?.salutation)}. ${residentData?.personal?.name}</b> (${formulaDescription}). Please note all payments must be made by the 5th of every month (as per our terms and conditions).</p>
...
"
class="welcomeLetter-footer privatePermanent-welcomeLetter"
>
