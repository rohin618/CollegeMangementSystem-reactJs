		...
		const INNER = (
			src ?
				...
				/> : <div style={{ width: size, height: size }}
					className={`bg-l${darkModeStatus ? 'o25' : '25'}-${color} text-${color} avatar rounded-circle border border-2 border-white d-flex align-items-center justify-content-center`}>
					<span className='fw-bold'>{getFirstLetter(userName ?? '')}</span>
				</div>
		);
