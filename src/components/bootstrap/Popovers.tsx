...
const Popovers = forwardRef<PopoverRef, IPopoversProps>(
	(
		{
			children,
			className,
		...
		// 🔗 expose reference element to parent
		useImperativeHandle(forwardedRef, () => ({
			referenceElement: referenceElement as HTMLSpanElement,
		...
		const { styles, attributes } = usePopper(referenceElement, popperElement, {
			...
			modifiers: [
				{
					name: 'offset',
					options: { offset: [0, 6.5] },
				},
				{
					name: 'flip',
					enabled: true,
					options: { fallbackPlacements: flip },
				},
				{
					name: 'arrow',
		...
		const ON_CLICK = () => {
			if (trigger === 'click') setPopoverOpen((p) => !p);
				...
				{popoverOpen && (
					<Portal>
						<div
							...
							{desc && (
								<div className={classNames('popover-body', bodyClassName)}>
									{desc}
								</div>
							)}
						</div>
